...
> **⚠️ Windows 11 の Smart App Control に注意**: 既定では未知の発行元のスクリプトをブロックします。初回は必ず引っ掛かります。引っ掛かった場合は **B方式**（手動インポート）が確実。詳しくは [FAQ](#6-よくある質問faq) の「Smart App Control...」を参照
...
**Q. Windows 11 で「Smart App Control が安全でない可能性のあるファイルをブロックしました」と出て install.vbs が実行できない**  
A. Windows 11 標準の **Smart App Control (SAC)** が未知の発行元のスクリプトをブロックしています。3つの選択肢：

  - 一番ラクで確実 → エクスポートで「**プロジェクト全体 (.zip)**」を使い、Excel の VBE 「ファイル → ファイルのインポート」で `.frm`/`.bas` を手動取り込み（[§4.7 B](#47-エクスポートして-excel-に持っていく) 参照）
  - install.vbs を使いたい → 右クリック → プロパティ → 下部「**ブロックの解除**」にチェック → OK → コマンドプロンプトを開いて `cscript install.vbs` と実行（ダブルクリックでなく cscript 経由だと通ることが多い）
  - SAC を完全にオフにする → Windows セキュリティ → アプリとブラウザの制御 → Smart App Control → オフ。**注意**: SAC は一度オフにすると Windows の再インストールでしか戻せません
...
- **Smart App Control (Windows 11) でブロックされる**: 上の FAQ「Windows 11 で『Smart App Control...』」を参照
