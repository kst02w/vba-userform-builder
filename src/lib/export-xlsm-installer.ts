...
## 注意
- 初回実行時、Excel の「VBA プロジェクトオブジェクトモデルへのアクセスを信頼する」を
  有効にする必要があります（オプション → トラスト センター → マクロの設定）
- インポート後、フォーム名や標準モジュール名は元のままです

## 手動で取り込む場合
Excel の VBE (Alt+F11) → ファイル → ファイルのインポート で
.frm / .bas / .cls を一つずつ追加できます
